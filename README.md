...
```
potsherd doctor --privacy · 23 aug 2026
  ...
  `--quiet` and `--json` suppress that line. The plugin's SessionEnd hook runs
  `index --quiet` without `--embed`, so it downloads nothing at all.
