...
Retrieval is bm25 over an fts5 index, plus cosine over the stored vectors,
fused with reciprocal rank — and then **calibrated**, which is a second and
separate axis. The fused score ranks; it cannot say whether anything matched
*well*, because reciprocal rank fusion is a function of rank alone. So every row
also carries `confidence: strong | weak | none`, computed from how much of your
query the row actually covers, how strongly it scored within its own list, and
how many lists agreed. Below a floor, `find` returns **nothing** and says so.
An honest empty is a usable answer; ten rows of maybes are not.

`--vectors auto` (the default) skips the vector half when the words already
matched, which is why the screen above says `bm25`. There is no model call in any
of it.
...
```
potsherd index · ~/.potsherd · 24 aug 2026
  ...
  full index                 875ms   228 parsed · 0 unchanged · 546 KB
...
looked in, present or not. Semantic search needs no flag and no
decision: the embedding runtime is fetched in the background on the first index,
`index` returns immediately with text search live, and `find` upgrades to hybrid
the moment the vectors are ready. While it warms, every `find` says so.

```bash
potsherd index                       # incremental; only what changed
potsherd index --no-embed             # text only — fetch nothing, ever
...
used those words. They cost a one-off runtime and model download that `index`
announces on one line and then does in the background, newest sessions first, so
the work you are most likely to ask about is searchable within the first minute.
You never ask for them and there is no tier to buy into: `--no-embed` is the
escape hatch for a machine that must fetch nothing, not the default.
...
```
potsherd doctor --privacy · 24 aug 2026
  ...
  these never do, and open no socket at all:
    audit, rescue, guard, ls, show, stats, tag, note, pin, unpin,
    link, setup, stack, ignore, unignore, doctor

  this one calls no model and does reach the network,
  because it acquires what it needs instead of asking you to:
    potsherd index     the embedding runtime, once per machine
  ...
  one other download, once per machine: the embedding runtime,
  fetched by potsherd index without being asked.
  The first `potsherd index` on a machine fetches from cdn.jsdelivr.net and
  huggingface.co — 46.1 MB of WebAssembly runtime and quantized model weights,
  into ~/.potsherd/models. Every file is pinned to a size and a sha256 that
  ships in the source and is checked before it is kept. They are GET requests
  for public files: no transcript, no path, no identifier and no count leaves
  this machine, and nothing is installed into node_modules. It runs in a
  background process so the verb returns as soon as text search is live, which
  means `--quiet` and `--json` — and the plugin's SessionEnd hook, which runs
  `index --quiet` — do it silently. `potsherd index --no-embed` does not do it
  at all, and neither does any run on a machine with `POTSHERD_OFFLINE` set;
...
| 8 | ghost and session titles, `ignore`, the amended eval gate, offline `index`, `ask --cheap`, the id inventory | — |
| 9 | version, changelog, three fresh-machine proofs, the release | **`v1.1.0`** |

The verifier that read each phase's work was never its author, and it has found
something every single time: **12 · 8 · 9 · 7 · 13 · 15 · 14 · 7 · 15 · 15**. That run has
