...
| F15 | **cursor's cwd IS recoverable from `~/.cursor` alone** — absolute paths in the session's own tool inputs, accepted only when `cursorSlug(candidate) === projectSlug`. This resolves both real projects, including the underscore case that slug inversion can never reach: a slug `Users-example-data-pipeline` whose directory is actually `/Users/example/data_pipeline` — the slug spells an underscore and a hyphen the same way, so inverting it is guesswork and reading the tool inputs is not. **(substituted 2026-08-22: the two names are synthetic; the real pair was a private project directory. The shape — hyphen-in-slug vs underscore-on-disk — and the finding are the measured ones.)** So the scope decision cost less than feared | T1.3b | |
