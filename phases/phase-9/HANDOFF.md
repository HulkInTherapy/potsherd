...
first command said *"once published — today it is a git clone"*, which cannot be corrected without a
`1.1.1` (it is generated by a `prepack` script now, so it cannot go stale again); **`14.5 s` in the
release notes was measured by nothing** — the recorded figures are 12.29 s and 14.49 s; and
**`GO-LIVE` step 5 attached a tarball nothing creates**, so the release would have failed *after*
the irreversible publish.

**Original status line, kept:** **Status at the time of writing: `v1.1.0` is prepared and NOT yet tagged.** The verifier's report,
