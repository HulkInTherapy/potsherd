...
| `src/summarizer.ts` | `packages/core/src/llm.ts` (phase 2, T2.1) | **three of four taken, one refused** | 594 lines. Not cheap: the prompt, the output contract and the sentinel wiring are all built around upstream's 2–4 sentence blob, whereas potsherd's is the `03` §6 card schema, and the file imports the generated `src/version.ts` and `file-lock.ts`, both refused. What *is* worth copying when phase 2 arrives is small and recorded here so it is not lost: the `query()` option set — `model` from env with a `sonnet` fallback ladder, `resume: sessionId` so the model sees the original session's context, `persistSession: false` (upstream #83, stops the SDK writing a fake session into `~/.claude/projects`), and the `EPISODIC_MEMORY_SUMMARIZER_GUARD` env re-entrancy guard (#87). **T2.1 update:** the `query()` option set, `persistSession: false` and the guard (as `POTSHERD_LLM_GUARD`) all landed in `llm.ts`. `resume: sessionId` was **refused on principle**: it hands the model the raw transcript off disk, which is the one text that has never been through `redact()`, so it would make "every string is redacted before it reaches a model" (`03` §5) unenforceable from inside `llm.ts`. It also needs a writable `~/.claude/projects`, which `03` §11 forbids. The card pipeline passes redacted slices instead. |
